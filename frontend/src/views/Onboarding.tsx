    ...
    <div style={{ minHeight: "100%", display: "flex", alignItems: "center", justifyContent: "center", padding: 40 }}>
      <div style={{ width: 720, maxWidth: "100%" }}>
        <h1 style={{ marginBottom: 4 }}>
          Welcome to <span style={{ color: "var(--accent)" }}>commit-graph</span><span style={{ color: "var(--accent2)" }}>er</span>
        </h1>
        ...
        {done ? (
          <div style={{ padding: 20, border: "1px solid #238636", borderRadius: 10, background: "var(--panel)" }}>
            <h3 style={{ marginTop: 0 }}>✓ Connected {done}</h3>
            <p style={{ color: "var(--muted)" }}>You can add several accounts (even multiple GitHub users / orgs).</p>
            <div style={{ display: "flex", gap: 10 }}>
              <button onClick={addAnother} style={{ ...box, width: "auto", cursor: "pointer" }}>+ Add another account</button>
              <button onClick={onDone} style={{ ...box, width: "auto", background: "var(--accent)", color: "#fff", border: "none", cursor: "pointer" }}>Go to the app →</button>
            </div>
          </div>
        ) : !picked ? (
          <>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 12 }}>
              {PROVIDERS.map((p) => (
                <button key={p.id} onClick={() => { setPicked(p); setMsg(null); }}
                  ...
                  <Logo p={p} />
                  <b>{p.name}</b>
                  <span style={{ fontSize: 11, color: p.supported ? "#3fb950" : "var(--muted)" }}>{p.supported ? "✓ ready" : "preview"}</span>
                </button>
              ))}
            </div>
          </>
          ...
          <>
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 4 }}>
              ...
              <h3 style={{ margin: 0 }}>2 · Connect {picked.name}</h3>
              <button onClick={() => setPicked(null)} style={{ ...box, width: "auto", padding: "4px 10px", cursor: "pointer" }}>← back</button>
            </div>
            <ol style={{ color: "var(--muted)", lineHeight: 1.6 }}>{picked.steps.map((s, i) => <li key={i}>{s}</li>)}</ol>
            {picked.tokenUrl !== "#" && (
              <a href={picked.tokenUrl} target="_blank" rel="noreferrer" style={{ color: "var(--accent)" }}>→ Open {picked.name} token page (scopes: {picked.scopes})</a>
            )}
            <div style={{ display: "grid", gap: 8, marginTop: 16 }}>
              ...
              <input style={box} type="password" placeholder="personal access token" value={form.token} onChange={(e) => setForm({ ...form, token: e.target.value })} />
              ...
              <button disabled={busy || !form.username || !form.token} onClick={connect} style={{ ...box, background: "#238636", color: "#fff", border: "none", cursor: "pointer" }}>
                {busy ? "Connecting…" : `Connect & sync ${picked.name}`}
              </button>
              {!picked.supported && <div style={{ color: "#d29922", fontSize: 12 }}>Note: {picked.name} adapter is a preview — sync may not work yet.</div>}
            </div>
          </>
        )}
        ...
        {!done && <div style={{ marginTop: 24 }}>
          <button onClick={onDone} style={{ background: "transparent", color: "var(--muted)", border: "none", cursor: "pointer", textDecoration: "underline" }}>Skip for now</button>
        </div>}
      </div>
    </div>
  );
